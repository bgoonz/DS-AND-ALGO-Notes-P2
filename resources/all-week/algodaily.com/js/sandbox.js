(function () {
  ...
  let SandBox = (window.SandBox = class {
    static create(options) {
      return new Promise((done) => {
        ...
        frame.src = options.environment
          ? `/${options.environment.replace(" ", "-")}-empty.html`
          : "/empty.html";
        if (options.place) {
          options.place(frame);
        } else {
    ...
    run(code, output, meta) {
      ...
      var self = this;
      setTimeout(function () {
        if (self.output.div.innerHTML == "<div></div>") {
          self.output.spinner();
        }
      }, 300);
    }

    executionProcessor(text) {
    ...
    setupEnv() {
      // jz: setting up event handlers to transmit back to main listener
      let win = this.win;
      win.__sandbox = this;
      ...
      // jz: for passing tests
      win.testsPassed = this.testsPassed;
  ...
  SandBox.Output = class {
    ...
    spinner() {
      this.div.innerHTML = "<i class='fas fa-spinner fa-pulse'></i>";
    }
